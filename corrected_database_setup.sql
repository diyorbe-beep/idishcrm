-- ==============================================
-- TO'G'IRLANGAN DATABASE SETUP - Supabase uchun
-- ==============================================
...
-- 11. Barcha mavjud storage policies'ni o'chirish
...
DROP POLICY IF EXISTS "Simple storage policy" ON storage.objects;
DROP POLICY IF EXISTS "Allow all operations for product images" ON storage.objects;

-- 12. RLS'ni butunlay o'chirish (rasm yuklash uchun)
ALTER TABLE storage.objects DISABLE ROW LEVEL SECURITY;
ALTER TABLE storage.buckets DISABLE ROW LEVEL SECURITY;

-- 13. Qo'shimcha demo mahsulotlar
INSERT INTO products (name, category, brand, price, cost_price, selling_price, discount, quantity, min_quantity, barcode, description) 
...
INSERT INTO products (name, category, brand, price, cost_price, selling_price, discount, quantity, min_quantity, barcode, description) 
SELECT 'Whirlpool Sovutgich', 'Sovutgichlar', 'Whirlpool', 2800000, 1960000, 2800000, 5, 10, 3, '8801234567895', '280L nofrost sovutgich'
WHERE NOT EXISTS (SELECT 1 FROM products WHERE barcode = '8801234567895');
...
INSERT INTO customers (name, phone, email, address, bonus_points, total_purchases) 
SELECT 'Javlon Umarov', '+998901234571', 'javlon@example.com', 'Toshkent, Mirobod tumani', 200, 3500000
WHERE NOT EXISTS (SELECT 1 FROM customers WHERE phone = '+998901234571');
...
INSERT INTO sales (customer_id, user_id, total, payment_method) 
SELECT 
  (SELECT id FROM customers WHERE phone = '+998901234570' LIMIT 1),
  (SELECT id FROM users WHERE email = 'sotuvchi@idishbozor.uz' LIMIT 1),
  3500000,
  'karta'
WHERE EXISTS (SELECT 1 FROM customers WHERE phone = '+998901234570')
AND EXISTS (SELECT 1 FROM users WHERE email = 'sotuvchi@idishbozor.uz');
...
INSERT INTO sale_items (sale_id, product_id, quantity, price, discount) 
SELECT 
  (SELECT id FROM sales WHERE total = 3500000 LIMIT 1),
  (SELECT id FROM products WHERE barcode = '8801234567893' LIMIT 1),
  1,
  3500000,
  8
WHERE EXISTS (SELECT 1 FROM sales WHERE total = 3500000)
AND EXISTS (SELECT 1 FROM products WHERE barcode = '8801234567893');
...
SELECT 
  ...
  'kirim',
  15,
  'Yangi kir yuvish mashinalari',
  (SELECT id FROM users WHERE email = 'admin@idishbozor.uz' LIMIT 1)
WHERE EXISTS (SELECT 1 FROM products WHERE barcode = '8801234567893')
AND EXISTS (SELECT 1 FROM users WHERE email = 'admin@idishbozor.uz');
...
INSERT INTO purchase_orders (supplier_id, total_amount, status, delivery_date, notes) 
SELECT 
  (SELECT id FROM suppliers WHERE name = 'LG Electronics' LIMIT 1),
  5950000,
  'kutilmoqda',
  NOW() + INTERVAL '14 days',
  'Konditsionerlar uchun buyurtma'
WHERE EXISTS (SELECT 1 FROM suppliers WHERE name = 'LG Electronics');
...
INSERT INTO purchase_order_items (order_id, product_id, quantity, unit_price) 
SELECT 
  (SELECT id FROM purchase_orders WHERE notes = 'Konditsionerlar uchun buyurtma' LIMIT 1),
  (SELECT id FROM products WHERE barcode = '8801234567891' LIMIT 1),
  5,
  5950000
WHERE EXISTS (SELECT 1 FROM purchase_orders WHERE notes = 'Konditsionerlar uchun buyurtma')
AND EXISTS (SELECT 1 FROM products WHERE barcode = '8801234567891');

-- 20. Categories RLS policies (ixtiyoriy)
...
-- ==============================================
-- SETUP TUGADI - ENDI RASM YUKLASH ISHLAYDI!
-- ==============================================
